...
fu! <SID>CommanderMappings()
	...
	noremap <silent> <buffer> <F12>            :cal VimCommanderToggle()<CR>
	"Panel-dirs
	...
	"cysh 10271250 
	"Diff
	noremap <silent> <buffer> <leader>ff    :cal <SID>VimCommander_Diff()<CR>
endf
...
fu! <SID>InitCommanderOptions()
	...
	"cysh 10261832 
	let g:vimcommander_selected_on_left=""
	let g:vimcommander_selected_on_right=""
endf
...
fu! <SID>InitCommanderColors()
	...
	if s:use_colors
		...
		syntax match VimCommanderDirLine "^..+.*" transparent contains=netrwDir
		...
		"cysh 10272335 
		hi link VimCommanderPath netrwDir
		hi link VimCommanderNode Comment
		"hi link OpsFile Question
		hi link VimCommanderFile netrwPlain
		hi link VimCommanderSource Question
		hi link VimCommanderHeader Include
		hi link VimCommanderSpecial Function
		hi link VimCommanderSelectedFile netrwDir
		hi link VimCommanderSelectedDir netrwDir
	en
endf
...
fu! <SID>RefreshDisplays()
	...
	norm 4|
	"if winbufnr(0) == s:bufnr_left
		"exe "set statusline=".s:path_left."/"
	"else
		"exe "set statusline=".s:path_right."/"
	"endif
	cal <SID>SwitchBuffer()
	let line=line('.')
	cal <SID>BuildTree(<SID>MyPath())
	...
	if  <SID>IsSelected(name)
		...
		while found!=""
			if found!=name
				...
				"cysh 10271052 
				if winbufnr(0) == s:bufnr_left
					if name == g:vimcommander_selected_on_left
						let g:vimcommander_selected_on_left = ""
					end
				else
					if name == g:vimcommander_selected_on_right
						let g:vimcommander_selected_on_right = ""
					end
				end
			end
		...
		"cysh 10271052 
		if winbufnr(0) == s:bufnr_left
			let g:vimcommander_selected_on_left=<SID>FilenameUnderCursor()
		else
			let g:vimcommander_selected_on_right=<SID>FilenameUnderCursor()
		end
		echom "left :".g:vimcommander_selected_on_left." right :".g:vimcommander_selected_on_right
		setl ma
		norm! |
		norm! l
		norm! s>
...
"cysh
fu! <SID>VimCommander_Diff()
	let fileleft = s:path_left.s:slash_char.g:vimcommander_selected_on_left
	let fileright = s:path_right.s:slash_char.g:vimcommander_selected_on_right
	
	if filereadable(fileleft) && filereadable(fileright)
		call <SID>VimCommander_DiffFile(fileleft, fileright)	
	elseif isdirectory(fileleft) && isdirectory(fileright)
		call <SID>VimCommander_DiffDir(fileleft, fileright)	
	else
		echoerr "  Unable to diff ".fileleft." ".fileright
	end
endf

fu! <SID>VimCommander_DiffFile(fileLeft, fileRight )
	exe "tabe ".a:fileRight
	exe "vert diffsplit ".a:fileLeft
endf

fu! <SID>VimCommander_DiffDir(dirLeft, dirRight)
	":DirDiff dirLeft dirRight	
	exe "tabe "
	exe "DirDiff ".a:dirLeft." ".a:dirRight
endf
